...
import 'dart:convert';
import 'exercise_recommender.dart'; // Import the exercise_recommender.dart file
import 'package:flutter_dotenv/flutter_dotenv.dart';

String backendBaseUrl = dotenv.env['BACKEND_BASE_URL']!;

class Modelback extends StatelessWidget {
  final DateTime? selectedDate;
  final DateTimeRange? selectedDateRange;

  const Modelback({Key? key, this.selectedDate, this.selectedDateRange}) : super(key: key);
...
class _InteractiveSvgPageState extends State<InteractiveSvgPage> {
  String svgPath = "assets/images/backmodel.svg"; // Path to SVG
  List<_PathData> paths = [];
  Map<String, String> groupToMuscle = {}; // Map to associate path IDs with muscle groups
  ...
  Future<void> fetchMusclePercentages() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print("Failed to load muscle percentages: ${response.statusCode}");
      }
    } catch (e) {
  ...
  void showExerciseBottomSheet(BuildContext context, String muscleGroup) {
    final exercises = recommendExercises(muscleGroup);  // Get the exercises for the muscle group

    // Show the bottom sheet with exercises
